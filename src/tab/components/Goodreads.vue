      ...
      >
        <span slot="label">
          <el-badge
            ...
            :hidden="!shelf.books.length"
            :type="shelf.badgeType"
            class="badge"
          >
            {{ shelf.title }}
          </el-badge>
...
const { GOODREADS_API, GOODREADS_USER_ID } = process.env;
  ...
  mounted() {
    for (let shelf of this.shelves) {
      setTimeout(
        ...
        shelf.name === this.activeShelfName ? 100 : 2500
      ); // load active shelf first, others later
    }
  },
  methods: {
    ...
    getShelf(shelfName) {
      ...
      if (books) {
        ...
        axios
          .get(
            `https://www.goodreads.com/review/list/${GOODREADS_USER_ID}.xml?shelf=${shelfName}&per_page=200&key=${GOODREADS_API}&v=2`
          )
          .then((res) => {
            // console.log(res)
            parseString(
              ...
              (err, result) => {
                // console.log(result)
                let books = result.GoodreadsResponse.reviews.review.map(
                  (r) => r.book
                );
                // console.log(books)
                this.putBooksToShelf(shelfName, books);
